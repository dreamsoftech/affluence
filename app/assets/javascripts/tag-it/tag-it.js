(function ($) {
    ...
    $.fn.tagit = function (options) {
        ...
        var settings = {
            ...
            'blurTrigger':false //when enabled, a tag will be created when the blur event fires
        };
        ...
        if (options.availableTags || options.tagSource) {
            tagInput.autocomplete({
                ...
                select:function (event, ui) {
                    var tag = create_tag(ui.item.value)
                    if (ui.item.profile_id) {
                        $(tag).attr('data-profile_id', ui.item.profile_id)
                    }
