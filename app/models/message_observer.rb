class MessageObserver < ActiveRecord::Observer
  ...
  def create_interaction_thread(message)
   ...
   interaction = Interaction.find(:last, :conditions => ["conversation_id = ?", message.conversation_id])
   ...
   if !interaction.blank?
     ...
     Interaction.create(:concierge_request_id => interaction.concierge_request_id, :interactable_id => message.id,  :interactable_type => 'Message', :conversation_id => message.conversation_id)
