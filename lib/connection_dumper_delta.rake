...
namespace :affluence do
  ...
  task :connection_dumper_delta_start, :start, :end, :needs => :environment do |t, args|
    ...
    connect_to_old_db
    ProdFriendship.find(:all,:conditions => "created_at > '#{args[:start]}' and created_at <= '#{args[:end]}'", :order => 'ID ASC').each do | friend_obj |
