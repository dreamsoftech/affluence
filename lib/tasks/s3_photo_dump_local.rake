...
namespace :affluence do
  ...
  task :s3_photo_dump_local => :environment do
    puts "Rake Started.."
    ...
    ProdUser.find(:all,:conditions => "id >= 2058",:order => 'ID ASC').each do | user_obj |
          ...
          if user_obj.status != 'invited' then
            ...
            unless _existing_user_obj.nil? then
              ...
              if _existing_user_obj.profile.photos.blank?
                path = "/home/seneca/Desktop/s3_covert/profile_pictures/"+photo_id.to_s+"/original/"+photo_name.to_s
